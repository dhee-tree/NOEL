...
class WishlistItemDetailAPIView(APIView):
    ...
    def get(self, request, wishlist_id, item_id, *args, **kwargs):
        """Return item details"""
        item = self.get_item_with_ownership_check(request, wishlist_id, item_id)
    ...
    def put(self, request, wishlist_id, item_id, *args, **kwargs):
        ...
        serializer = UpdateWishlistItemSerializer(item, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    def patch(self, request, wishlist_id, item_id, *args, **kwargs):
        """Update item partially"""
        item = self.get_item_with_ownership_check(request, wishlist_id, item_id)
        
        serializer = UpdateWishlistItemSerializer(item, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
